    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                # success_ret_[video.name] = success_overlap(gt_traj, tracker_traj, n_frame)  # orignal
                if 'HOB' in self.dataset.name:
                    success_ret_[video.name] = success_overlap(gt_traj[::15], tracker_traj[::15], n_frame/15)  # xyl 20230405
                else:
    ...
    def eval_precision(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                # precision_ret_[video.name] = success_error(gt_center, tracker_center,  # orignal
                #         thresholds, n_frame)
                if 'HOB' in self.dataset.name:
                    precision_ret_[video.name] = success_error(gt_center[::15], tracker_center[::15],  # xyl 20230405
                                                               thresholds, n_frame/15)
                else:
