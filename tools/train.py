...
def build_opt_lr(model, current_epoch=0):
    ...
    # trainable_params += [{'params': model.gal.parameters(),  # xyl 20221003
    #                       'lr': cfg.TRAIN.BASE_LR}]

    # trainable_params += [{'params': model.cam.parameters(),  # xyl 20230303 CAM
    #                       'lr': cfg.TRAIN.BASE_LR}]
...
def main():
    ...
    if rank == 0:
        ...
        if cfg.TRAIN.LOG_DIR:
            loca = time.strftime('%Y-%m-%d-%H-%M-%S)')  # 获取时间作为保存的文件名
            add_file_handler('global',
                             os.path.join(cfg.TRAIN.LOG_DIR, 'logs_{}.txt'.format(loca)),
                             logging.INFO)
    ...
    if cfg.BACKBONE.PRETRAINED:
        ...
        backbone_path = os.path.join('/home/xyl/pysot-master/tools', cfg.BACKBONE.PRETRAINED)
