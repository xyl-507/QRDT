...
def get_frames(video_name):
    if not video_name:
        ...
        images = sorted(images,
                        key=lambda x: x.split('/')[-1].split('.')[0])  # suitable for img0000x.jpg
        for img in images:
        ...
        if first_frame:
            ...
            if 'polygon' in outputs:
                ...
                cv2.rectangle(frame, (bbox[0], bbox[1]),
                              ...
                              (0, 255, 0), 3)
            cv2.putText(frame, '#{}'.format(i), (10, 30), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 255), 2)
            cv2.imshow(video_name, frame)
            cv2.waitKey(40)
            cv2.imwrite("/home/xyl/xyl-code/siamban-DROL/demo/results/{:06d}.jpg".format(i), frame)
        i += 1
