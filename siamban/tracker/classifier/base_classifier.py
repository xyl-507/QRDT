...
class BaseClassifier(object):
    ...
    def track(self, scale_idx=None):
        ...
        self.feat_x = self.extract_processed_sample(scale_idx=scale_idx)  # 根据cf（也就是搜索特征xf）得到分类响应图的

        # Compute scores
        scores_raw = self.apply_filter(self.feat_x)  # ATOM的分类器
    ...
    def project_sample(self, x: TensorList, proj_matrix = None):  # 压缩模块和注意力模块
    ...
    def extract_processed_sample(self, im: torch.Tensor = None, scale_idx=None):
        ...
        feat = self.project_sample(x)  # 压缩模块和注意力模块
    ...
    def init_memory(self, train_x):
        ...
        self.init_sample_weights = TensorList([x.new_ones(1) / x.shape[0] for x in train_x])  # ​torch.new_*​​ 用于创建一个与已知 tensor 类型相同的 tensor，new_ones是用1填充
        self.init_training_samples = train_x

        # Sample counters and weights
        self.num_stored_samples = self.num_init_samples.copy()
        self.previous_replace_ind = [None] * len(self.num_stored_samples)
        self.sample_weights = TensorList([x.new_zeros(cfg.TRACK.SAMPLE_MEMORY_SIZE) for x in train_x])  # new_zeros是用0填充
