...
class GradientDescent:
    ...
    def run(self, num_iter, dummy = None):
        ...
        for i in range(num_iter):
            ...
            # grad = torch.autograd.grad(loss[0], self.x, grad_outputs=torch.ones_like(loss[0]))
            # grad = TensorList(grad)
            # grad = TensorList(torch.autograd.grad(loss, self.x), grad_outputs=torch.ones_like(loss))
